...
describe('', function () {
    ...
    function uploadFileAsync(imageFileAddress) {
        var fileName = imageFileAddress.replace('http://weed.cocolog-nifty.com/wzero3es/', '').replace('/', '_');
        ...
        // チェックボックスがオフならオンにする
        $('input#keep_original').isSelected().then(function (bool) {
            if (!bool)
                $('input#keep_original').click();
        });
        ...
        $xpath('//dt[text()="HTMLタグ"]/../dd[1]/textarea').getText().then(function (text) {
            console.log(text);
            return text;
        });
    }
